...
(function () {
    ...
    /* --------------------------------------------------------------- config
       Everything environment-specific lives here. Every internal link is
       relative and every asset is local, so this folder runs unchanged from
       any domain, any sub-path, or straight off a USB stick over file://. */
    const CONFIG = {
        contact: 'oasis@labidi.eu',
        home: 'oasis.labidi.eu',
        parentSite: 'https://labidi.eu/',   // set to null to drop the chip entirely
        parentName: 'labidi.eu',
    };
    ...
    function pageAbout() {
        ...
        g.appendChild(el('div', { class: 'card' }, [
            ...
            el('p', { class: 'note', text: 'This system lives at ' + CONFIG.home + '. Everything in it is relative and self-contained, so an installed copy, a printed page and a folder on a USB stick all keep working whatever happens to the website.' }),
        ]));
    ...
    function boot() {
        ...
        /* The parent-site chip is configuration, not markup. Setting
           CONFIG.parentSite to null removes it entirely. */
        const parent = $('#parentLink');
        if (parent) {
            if (CONFIG.parentSite) {
                parent.href = CONFIG.parentSite;
                parent.textContent = '↖ ' + CONFIG.parentName;
                parent.title = 'More projects at ' + CONFIG.parentName;
            } else {
                parent.remove();
            }
        }
